...
for patpath in $(ls -d ${PLANSDIR}/P* | grep -E "P05"); do
    patient=$(basename ${patpath})
    echo "Creating DVH for ${patient}"

    for dose in ${patpath}/cbct_?/out_cbct*/ast_total.mhd; do
        case=$(basename $(dirname $(dirname ${dose})))
        ...
        plastimatch dvh \
            ...
            --output-csv    "${DVHDIR}/${patient}_${case}.dvh"           \
            --dose-units    "gy"                                         \
            --num-bins      "901"                                        \
            --bin-width     "0.1"                                        \
